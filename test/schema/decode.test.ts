...
function valid<T>(result: Validation<T, DecodeError>): T {
    if (getTag(result) !== "Valid")
        throw new Error(`Expected Valid, got ${getTag(result)}`);
    return (result as Valid<T, DecodeError>).value;
}

function errors(result: Validation<any, DecodeError>): DecodeError[] {
    if (getTag(result) !== "Invalid")
        throw new Error(`Expected Invalid, got ${getTag(result)}`);
...
describe("Schema factory", () => {
    ...
    it("Schema.object() creates an ObjectSchema variant", () => {
        expect(getTag(Schema.object({ name: Schema.string() }))).toBe(
            "ObjectSchema",
        );
    });
    it("Schema.union() creates a UnionSchema variant", () => {
        expect(getTag(Schema.union(Schema.string(), Schema.number()))).toBe(
            "UnionSchema",
        );
    });
    it("Schema.variant() creates a VariantSchema variant", () => {
        const F = union({ A: () => ({}) });
        expect(getTag(Schema.variant(F, { A: Schema.object({}) }))).toBe(
            "VariantSchema",
        );
    });
...
describe("decode: number", () => {
    ...
    it("rejects a string", () => {
        expect(getTag(errors(decode(Schema.number(), "42"))[0])).toBe(
            "TypeMismatch",
        );
    });
    it("infers number output type", () => {
        expectTypeOf(decode(Schema.number(), 1)).toExtend<
            Validation<number, DecodeError>
        >();
    });
});
...
describe("decode: boolean", () => {
    it("accepts true", () =>
        expect(valid(decode(Schema.boolean(), true))).toBe(true));
    it("accepts false", () =>
        expect(valid(decode(Schema.boolean(), false))).toBe(false));
    it("rejects a string", () => {
        expect(getTag(errors(decode(Schema.boolean(), "true"))[0])).toBe(
            "TypeMismatch",
        );
    });
});
...
describe("decode: literal", () => {
    ...
    it("infers literal output type", () => {
        expectTypeOf(decode(Schema.literal("ok"), "ok")).toExtend<
            Validation<"ok", DecodeError>
        >();
    });
});
...
describe("decode: optional", () => {
    ...
    it("accepts undefined", () =>
        expect(valid(decode(s, undefined))).toBeUndefined());
    it("rejects null", () => {
        expect(getTag(errors(decode(s, null))[0])).toBe("TypeMismatch");
    });
    it("infers T | undefined type", () => {
        expectTypeOf(decode(s, "x")).toExtend<
            Validation<string | undefined, DecodeError>
        >();
    });
});
...
describe("decode: nullable", () => {
    ...
    it("infers T | null type", () => {
        expectTypeOf(decode(s, null)).toExtend<
            Validation<string | null, DecodeError>
        >();
    });
});
...
describe("decode: nullish", () => {
    ...
    it("infers T | null | undefined type", () => {
        expectTypeOf(decode(s, null)).toExtend<
            Validation<string | null | undefined, DecodeError>
        >();
    });
});
...
describe("decode: array", () => {
    ...
    it("infers T[] output type", () => {
        expectTypeOf(decode(s, [])).toExtend<
            Validation<number[], DecodeError>
        >();
    });
});
...
describe("decode: object (strip mode)", () => {
    ...
    it("decodes a matching object", () => {
        expect(valid(decode(s, { name: "Alice", age: 30 }))).toEqual({
            name: "Alice",
            age: 30,
        });
    });
    it("strips extra keys by default", () => {
        const result = valid(
            decode(s, { name: "Alice", age: 30, extra: true }),
        );
    ...
    it("infers the object shape type", () => {
        expectTypeOf(decode(s, {})).toExtend<
            Validation<{ name: string; age: number }, DecodeError>
        >();
    });
});
...
describe("decode: object (optional fields)", () => {
    ...
    it("succeeds when optional field is absent", () => {
        expect(valid(decode(s, { name: "Alice" }))).toEqual({
            name: "Alice",
            age: undefined,
        });
    });
...
describe("decode: union", () => {
    ...
    it("infers union output type", () => {
        expectTypeOf(decode(s, "x")).toExtend<
            Validation<string | number, DecodeError>
        >();
    });
});
...
describe("decode: variant with map", () => {
    ...
    it("maps failure to Err", () => {
        const result = valid(
            decode(s, { status: "failure", message: "not found" }),
        );
...
describe("decode: nested object", () => {
    ...
    it("decodes deeply nested structures", () => {
        const result = valid(
            decode(s, { user: { name: "Alice", address: { zip: "10001" } } }),
        );
...
describe("decode: array of objects", () => {
    ...
    it("decodes an array of objects", () => {
        expect(valid(decode(s, [{ id: 1 }, { id: 2 }]))).toEqual([
            { id: 1 },
            { id: 2 },
        ]);
    });
